...
mermaid: true
highlight_anchor: branch-conflict-diagram
---
...
## 번외: 머지 충돌은 이렇게 생겼다 {: #branch-conflict-diagram}

이번엔 운 좋게 충돌 없이 넘어갔지만, 브랜치를 나눠 쓰다 보면 언젠가는 마주치게 된다.
미리 감을 잡아두려고 충돌이 발생하고 해결되는 과정을 그림으로 정리해봤다.

{% include embeds/branch-conflict-explainer.html %}
...
- **머지 충돌 해결** — 위 그림에서 다룬 건 가장 단순한 한 줄 충돌이다. 실제로는 같은 파일 여러 줄, 혹은 파일 자체가 삭제/이동된 경우처럼 더 복잡한 충돌도 생긴다. `git merge --abort`로 되돌리는 법까지 익혀두면 좋다.
